...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --mongo-version) MONGO_VERSION="$2"; shift 2 ;;
    --mongo-image)   MONGO_IMAGE="$2"; shift 2 ;;
    --go-version)    GO_VERSION="$2"; shift 2 ;;
    --rebuild-all)   REBUILD_ALL=1; shift ;;
    --smoke)         SMOKE=1; shift ;;
...
[[ -d "$PCSM_PATH"           ]] || { echo "PCSM_PATH not a dir: $PCSM_PATH" >&2; exit 1; }
[[ -d "$PYTEST_DIR"          ]] || { echo "pcsm-pytest dir missing: $PYTEST_DIR (set PSMDB_TESTING_PATH)" >&2; exit 1; }
[[ -f "$PYTEST_DIR/Dockerfile-csync-local" ]] || { echo "Dockerfile-csync-local missing in $PYTEST_DIR" >&2; exit 1; }
