...
- `lastReplicatedOpTime` is the reported frontier; `checkpointOpTime` is the inclusive resume floor, not proof the event at that timestamp was applied. Replay detection keeps strict `<`.
- `pool.Checkpoint` skips never-routed workers, uses the first routed timestamp for workers with no commit, and otherwise uses their committed timestamp; the minimum is the safe floor. A run-owned tracker advances the reported frontier and resume floor from it every 500ms, independently of dispatcher activity; cleanup stops and joins the tracker before stopping the pool. `Route`'s per-worker publishes and the `Checkpoint` scan are mutually exclusive under `routeMu`, so a scan never skips a worker that a concurrent `Route` is making visible.
