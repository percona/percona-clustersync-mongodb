...
func TestCreateView_Idempotency(t *testing.T) {
	...
	require.Contains(t, cat.Databases[db].Collections, view,
		"View should be tracked in catalog after first call")

	err = cat.CreateCollection(ctx, db, view, opts)
	assert.NoError(t, err, "Second CreateView call should be idempotent")

	assert.Contains(t, cat.Databases[db].Collections, view,
		"View should still be tracked in catalog after idempotent call")
}

// TestCreateView_CatalogTracking verifies that views are added to the catalog
// so that downstream operations (SetCollectionUUID, Checkpoint/Recover) work
// correctly. Without catalog tracking, SetCollectionUUID silently drops the
// UUID mapping which breaks change stream event resolution during replication.
func TestCreateView_CatalogTracking(t *testing.T) {
	t.Parallel()
	...
	require.Contains(t, cat.Databases, db)
	require.Contains(t, cat.Databases[db].Collections, view,
		"View must be tracked in catalog for SetCollectionUUID to work")

	testUUID := &bson.Binary{Subtype: 4, Data: []byte("0123456789abcdef")}
	cat.SetCollectionUUID(ctx, db, view, testUUID)

	uuidMap := cat.UUIDMap()
	found := false

	for _, ns := range uuidMap {
		if ns.Database == db && ns.Collection == view {
			found = true
	...
	assert.True(t, found, "View should appear in UUIDMap after SetCollectionUUID")

	cat.LockWrite()
	cp := cat.Checkpoint()
	cat.UnlockWrite()

	require.NotNil(t, cp, "Checkpoint should not be nil when catalog has entries")

	cat2 := catalog.NewCatalog(client)

	err = cat2.Recover(cp)
	require.NoError(t, err)

	assert.Contains(t, cat2.Databases[db].Collections, view,
		"View should survive checkpoint/recover")
}
