...
func TestResolveCloneSegmentSize_ConfigFallbackValidation(t *testing.T) {
	t.Parallel()

	// Test that config fallback values are also validated
	tests := []struct {
		name    string
		cfg     *config.Config
		wantErr string
		...
		{
			name: "config has invalid segment size (too small)",
			cfg: &config.Config{
				Clone: config.CloneConfig{
					SegmentSize: "1MB",
				},
			},
			wantErr: "config clone-segment-size",
		},
		{
			name: "config has invalid segment size (too large)",
			cfg: &config.Config{
				Clone: config.CloneConfig{
					SegmentSize: "100GiB",
				},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := config.ResolveCloneSegmentSize(tt.cfg, nil)
			require.Error(t, err)
			assert.Contains(t, err.Error(), tt.wantErr)
		})
	}
